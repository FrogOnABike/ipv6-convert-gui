...
class Window:
    def __init__(self,width,height):
        self.__root = Tk()
        self.__root.title("IPv6 Converter")
        ...
        ttk.Button(self.__frame,text="Process", command=self._process_ipv6_callback).grid(column=3,row=3,sticky=W)
       
        ttk.Label(self.__frame,text="IPv6 Address").grid(column=1,row=1,sticky=(W,E))
        ttk.Label(self.__frame, text="Result Address:").grid(column=1,row=2,sticky=E)
        
        for child in self.__frame.winfo_children():
            child.grid_configure(padx=5,pady=5)
        
        ip_add_entry.focus()
        self.__root.bind("<Return>",self._process_ipv6_callback)
        # self.converted_add.set(compress_ipv6(str(self.ip_add)))
        
        self.__root.mainloop()

    def _process_ipv6_callback(self, event=None): # event=None makes it compatible with both button command and bind
        """
        ...
        if len(full_address) == 39:
            ...
            try:
                compressed_address = expand_ipv6(full_address) # Call the imported function
                self.converted_add.set(f"{compressed_address}")
            except ValueError as e: # Catch specific errors from your compress_ipv6 if it raises them
                messagebox.showerror("Validation Error", str(e))
                self.converted_add.set("(Error)")
