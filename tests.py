...
class Tests(unittest.TestCase):
    ...
    def test_compression_examples(self):
        ...
        for address, expected_result in test_cases:
            with self.subTest(address=address): # subTest helps identify which specific test case failed
                actual_compressed = compress_ipv6(address)
                self.assertEqual(actual_compressed, expected_result, 
                                    f"Failed for {address}: Expected {expected_result}, Got {actual_compressed}")
    
    def test_expansion_examples(self):
        # Your existing test cases:
        # self.assertEqual(compress_ipv6_address("0000:0000:0000:0000:0000:0000:0000:0001"), "::1")
        # self.assertEqual(compress_ipv6_address("your_random_interview_address_full"), "your_random_interview_address_compressed")
        ...
        test_cases = [
            ...
            # 2. Zeros at the beginning
            ("::1234:5678:9abc:def0","0000:0000:0000:0000:1234:5678:9abc:def0"),
            
            # 3. Zeros at the end
            ("fe80::202:b3ff:fe1e:8329","fe80:0000:0000:0000:0202:b3ff:fe1e:8329", ),
            
            # 4. No contiguous zero blocks for '::', only leading zero omission
            ("2001:db8:85a3:1:1:8a2e:370:7334","2001:0db8:85a3:0001:0001:8a2e:0370:7334"),
            
            # 5. Multiple zero blocks, but only one '::' possible
            ("2001:db8:0:1234::5678:0","2001:0db8:0000:1234:0000:0000:5678:0000"),
        ...
        for address, expected_result in test_cases:
            with self.subTest(address=address): # subTest helps identify which specific test case failed
                actual_compressed = expand_ipv6(address)
                self.assertEqual(actual_compressed, expected_result, 
                                    f"Failed for {address}: Expected {expected_result}, Got {actual_compressed}")
